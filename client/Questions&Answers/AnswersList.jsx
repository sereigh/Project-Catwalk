...
function Answerslist(props) {
  ...
  const buttonText =
    answers.length > 2 ? "COLLAPSE ANSWERS" : "LOAD MORE ANSWERS";
      ...
      {answers.map((answer, i) => (
        ...
        >
          <span className="answerText">
            <strong>A: </strong>
            {query !== "" && answer.body.includes(query) ? (
              <SearchHighlight query={query} body={answer.body} />
            ) : (
              answer.body
            )}
          </span>
          <span className="answersFeedback">
            <UserInfo name={answer.answerer_name} date={answer.date} />
            {`   Helpful? `}
            <TextLink
              option={1}
